...
`mypy --strict` runs in CI and is blocking — confirmed passing clean
(`Success: no issues found in 19 source files`, 2026-09-09; one missing return-type
annotation in `src/forecasting.py`'s `_get_gradient_boosted_regressor` was the only
issue found and is now fixed).
